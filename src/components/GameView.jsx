...
export default function GameView({ gamePin }) {
  ...
  const [pin, setPin] = useState(gamePin || '')
  const audioRef = useRef(null)
  const clientRef = useRef(null)

  const connect = (gamePinValue) => {
    if (clientRef.current) { destroyClient(clientRef.current); clientRef.current = null }
    const gp = gamePinValue.trim().toUpperCase()
    setPin(gp)
    setPhase('connecting')
    setFadeIn(true)
    ...
    connectToHost(gp, 'display').then(client => {
      ...
      onClientData(client.conn, (data) => {
        if (data.type === 'playerList') {
          ...
          setTimeLeft(data.timeLimit || TIMER_DURATION)
          setAnswerCount(0)
        ...
        } else if (data.type === 'answerCount') {
          setAnswerCount(data.count || 0)
          setTotalPlayers(data.total || players.length)
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#0a0a1a] via-[#0f0f2a] to-[#1a0a2e] flex items-center justify-center p-4">
        <div className="text-center space-y-4">
          <div className="flex gap-2 justify-center">
            {[0,1,2].map(i => <span key={i} className="w-4 h-4 rounded-full bg-white/20 animate-bounce" style={{animationDelay:`${i*0.2}s`}} />)}
          </div>
  ...
  if (phase === 'lobby' && !pin) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#0a0a1a] via-[#0f0f2a] to-[#1a0a2e] flex items-center justify-center p-4">
        <div className="bg-white/[0.03] backdrop-blur-xl rounded-2xl border border-white/[0.06] p-8 text-center max-w-sm w-full space-y-4 shadow-2xl">
          <span className="text-5xl block">📺</span>
          <h2 className="text-2xl font-bold text-white">Game Display</h2>
          <p className="text-white/40 text-sm">Enter the game PIN to show on screen</p>
          <input value={pin} onChange={e => setPin(e.target.value.toUpperCase())}
            placeholder="PIN"
            className="w-full bg-white/[0.06] border border-white/[0.1] rounded-xl px-4 py-3 text-white text-center text-2xl font-black tracking-[0.2em] outline-none focus:border-purple-500/50 transition-all placeholder:text-white/20"
            maxLength={4}
            onKeyDown={e => e.key === 'Enter' && connect(pin)} />
          <button onClick={() => connect(pin)} disabled={!pin.trim()}
            className="w-full py-3 bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl font-bold text-white shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-40">
            Connect
          </button>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#0a0a1a] via-[#0f0f2a] to-[#1a0a2e] flex items-center justify-center relative overflow-hidden">
        <div className="absolute inset-0">
          {[...Array(15)].map((_, i) => (
            <div key={i} className="particle" style={{
              ...
              animationDelay: `${Math.random() * 5}s`, background: `rgba(255,255,255,${Math.random() * 0.1})`,
            }} />
          ))}
        </div>
        <div className="relative z-10 text-center space-y-6">
          ...
          <p className="text-2xl text-white/40 font-light">PIN: <span className="text-4xl font-bold text-yellow-400 tracking-widest">{pin}</span></p>
          <p className="text-xl text-white/40 font-light">Waiting for game to start...</p>
          {players.length > 0 && (
              ...
              <div className="flex justify-center gap-2 flex-wrap max-w-md mx-auto">
                {players.map(p => (
                  <div key={p.id} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/[0.04] rounded-full border border-white/[0.06]">
                    <AvatarDisplay avatar={p.avatar} imgClass="w-6 h-6 rounded-full" />
        ...
        <main className="flex-1 flex flex-col items-center justify-center px-8">
          <div className="max-w-4xl w-full text-center space-y-8">
            <div className="space-y-4">
              <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-white leading-tight">{currentQuestion.question}</h2>
            ...
            {currentQuestion.type === 'artist-info' && (
              <div className="flex items-center justify-center gap-4">
                <div className="w-20 h-20 rounded-full bg-gradient-to-br from-purple-500/30 to-pink-500/30 flex items-center justify-center border-2 border-white/10"><span className="text-3xl">🎤</span></div>
            ...
            {currentQuestion.type === 'grammar' && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
                {currentQuestion.options.map((opt, i) => (
                  <div key={i} className="bg-white/[0.04] rounded-2xl p-4 border border-white/[0.06] text-center" style={{ borderTop: `3px solid ${COLORS[i]}` }}>
        ...
        <footer className="px-8 pb-8">
          <div className="max-w-3xl mx-auto space-y-2">
            <div className="relative w-full h-4 bg-white/[0.06] rounded-full overflow-hidden">
              <div className="h-full rounded-full transition-all duration-1000 ease-linear" style={{ width: `${timerPct}%`, backgroundColor: timerColor, boxShadow: `0 0 20px ${timerColor}40` }} />
            </div>
            <div className="flex items-center justify-between">
              <span className="text-white/30 text-sm">{total - answeredCount} left</span>
              <span className={`text-5xl font-black tabular-nums transition-colors duration-300 ${isCritical ? 'text-red-500 animate-pulse' : isLow ? 'text-yellow-400' : 'text-white'}`}>{timeLeft}</span>
            </div>
          </div>
        </footer>
      </div>
        ...
        <div className="max-w-5xl mx-auto w-full space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            {currentQuestion?.options.map((opt, i) => {
              ...
              return (
                <div key={i} className={`rounded-2xl overflow-hidden transition-all duration-500 ${isCorrect ? 'ring-2 ring-emerald-500/50 scale-[1.02]' : ''}`} style={{ backgroundColor: COLORS[i] + '15', border: `1px solid ${COLORS[i]}30` }}>
                  <div className="p-5 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-lg font-black" style={{ color: COLORS[i] }}>{['A','B','C','D'][i]}</span>
                      {isCorrect && <span className="text-xs font-bold text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-full">✓ Correct</span>}
                    </div>
                    ...
                    <div className="w-full bg-white/[0.06] rounded-full h-4 overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-1000 ease-out" style={{ width: `${pct}%`, backgroundColor: COLORS[i] }} />
                    </div>
                    <div className="flex items-center justify-between">
            ...
            <div className="bg-white/[0.03] rounded-2xl border border-white/[0.06] overflow-hidden">
              ...
              <div className="divide-y divide-white/[0.04]">
                {sortedPlayers.slice(0, 10).map((p, i) => (
                  <div key={p.id} className={`flex items-center justify-between px-5 py-4 transition-all ${i === 0 ? 'bg-gradient-to-r from-yellow-500/10 to-amber-500/5' : ''}`}>
                    <div className="flex items-center gap-4">
                      <span className={`w-10 h-10 rounded-xl flex items-center justify-center font-black ${i === 0 ? 'bg-yellow-500/20 text-yellow-400 text-lg' : i === 1 ? 'bg-gray-300/20 text-gray-300 text-lg' : i === 2 ? 'bg-amber-600/20 text-amber-500 text-lg' : 'bg-white/[0.06] text-white/40'}`}>{i + 1}</span>
                      <AvatarDisplay avatar={p.avatar} imgClass="w-10 h-10 rounded-full" />
                      <div>
        ...
        <div className="absolute inset-0 pointer-events-none">
          {[...Array(30)].map((_, i) => (
            <div key={i} className="particle" style={{
              left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%`,
              width: `${Math.random() * 4 + 2}px`, height: `${Math.random() * 4 + 2}px`,
              animation: `float ${Math.random() * 10 + 6}s ease-in-out infinite`, animationDelay: `${Math.random() * 6}s`,
              background: i % 2 === 0 ? 'rgba(250,204,21,0.15)' : 'rgba(168,85,247,0.1)',
            }} />
          ))}
        </div>
        <div className="relative z-10 text-center space-y-8">
          <div className="inline-flex items-center justify-center w-24 h-24 rounded-3xl bg-gradient-to-br from-amber-500/20 to-yellow-500/20 border border-amber-500/20"><span className="text-6xl">🏆</span></div>
          <h1 className="text-5xl md:text-7xl font-black bg-gradient-to-r from-amber-400 via-yellow-400 to-orange-400 bg-clip-text text-transparent">GAME OVER</h1>
          {topPlayers.length > 0 && (
